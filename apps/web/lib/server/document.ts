export function parseDashboardDocument(value: unknown): Record<string, unknown> {
  ...
  if (![3, 4].includes(Number(item.schemaVersion))) throw new Error("Dashboard-Schema wird nicht unterstützt");
  ...
  if (item.schemaVersion === 4 && !Array.isArray(item.actions)) throw new Error("Aktionen sind ungültig");
