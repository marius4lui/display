...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: version } = await admin.from("display_versions").select("version, envelope, content_hash, published_at").eq("display_id", display.id).eq("version", display.active_version).maybeSingle();
  if (!version) return apiError("NOT_FOUND", "Kein veröffentlichtes Dashboard", 404);
  ...
  return NextResponse.json({ id, version: version.version, publishedAt: version.published_at, envelope: version.envelope }, { headers: { ETag: etag, "Cache-Control": "no-cache" } });
}
