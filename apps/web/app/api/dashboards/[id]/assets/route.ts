...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return error ? apiError("UPLOAD_FAILED", "Asset konnte nicht gespeichert werden", 500) : NextResponse.json({ id, url: publicUrl(request, `/assets/${id}`), contentHash }, { status: 201 });
}
