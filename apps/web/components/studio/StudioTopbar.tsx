...
export function StudioTopbar({ name, workspace, status, busy, onName, onWorkspace, onPreview, onSave, onPublish }: {
  name: string; workspace: "dashboard" | "api" | "integrations" | "custom-ui"; status: string; busy: boolean;
  onName: (name: string) => void; onWorkspace: (workspace: "dashboard" | "api" | "integrations" | "custom-ui") => void;
  ...
  return <header className="studio-topbar">
    ...
    <nav className="workspace-nav" aria-label="Arbeitsbereiche"><button className={workspace === "dashboard" ? "active" : ""} onClick={() => onWorkspace("dashboard")}>Dashboard</button><button className={workspace === "custom-ui" ? "active" : ""} onClick={() => onWorkspace("custom-ui")}>Custom UI</button><button className={workspace === "api" ? "active" : ""} onClick={() => onWorkspace("api")}>API Studio</button><button className={workspace === "integrations" ? "active" : ""} onClick={() => onWorkspace("integrations")}>Integrationen</button></nav>
