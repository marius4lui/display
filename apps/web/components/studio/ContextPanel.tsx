...
export type DeviceSummary = { id: string; name: string; platform?: "android" | "web"; online: boolean; last_seen_at?: string; app_version?: string; platform_version?: string; dashboard_version?: number; last_error?: string; revoked_at?: string };
export type PairingQr = { dataUrl: string; launchUrl: string; expiresAt: string; playerUrl?: string };
      ...
      {activity === "project" && <>
        ...
        {projectSection === "devices" && <div className="form-stack">
          {!dashboardId ? <div className="panel-empty"><Icon name="project"/><strong>Dashboard speichern</strong><p>Geräte können verbunden werden, sobald das Dashboard gespeichert ist.</p></div> : <>
            <section className="panel-section"><h3>Gerät koppeln</h3><p className="section-note">Der Code gilt einmalig zehn Minuten für Android und den Web-Player.</p><button className="primary-button full" disabled={busy} onClick={onPair}>Kopplungscode erzeugen</button>{pairingQr && <div className="pairing-qr"><img src={pairingQr.dataUrl} alt={`QR-Code für ${document.name}`}/><strong>Mit der display-App scannen</strong><small>Öffnet die Web-Übergabe und meldet das Android-Gerät automatisch an.</small><small>Gültig bis {new Date(pairingQr.expiresAt).toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit" })} Uhr.</small><a href={pairingQr.launchUrl}>Auf diesem Gerät öffnen</a></div>}{pairingCode && <div className="pairing-code"><small>Code für Android oder Web-Player</small><strong>{pairingCode}</strong>{pairingQr?.playerUrl && <a href={pairingQr.playerUrl} target="_blank" rel="noreferrer">Web-Player öffnen</a>}</div>}</section>
            <section className="device-list"><h3>Gekoppelte Geräte <span>{devices.length}</span></h3>{devices.length === 0 && <div className="section-empty">Noch keine Geräte gekoppelt.</div>}{devices.map((device) => <article className="device-card" key={device.id}><header><span className={device.online ? "device-online" : "device-offline"}/><div><strong>{device.name}</strong><small>{device.platform === "web" ? "Web" : "Android"} · {device.online ? "Online" : "Offline"}{device.last_seen_at ? ` · ${new Date(device.last_seen_at).toLocaleString("de-DE")}` : ""}</small></div></header><dl><div><dt>App</dt><dd>{device.app_version || "—"}</dd></div><div><dt>Plattform</dt><dd>{device.platform_version || "—"}</dd></div><div><dt>Dashboard</dt><dd>{device.dashboard_version ? `v${device.dashboard_version}` : "—"}</dd></div></dl>{device.last_error && <p>{device.last_error}</p>}{!device.revoked_at && <button className="danger-outline" onClick={() => onRevokeDevice(device.id)}>Zugriff widerrufen</button>}</article>)}</section>
          </>}
        </div>}
