...
import { starterCustomUi, validateCustomUi, type CustomUiDocument } from "../lib/custom-ui";
...
export default function CustomUiStudio({ document, data, onDocument, onNotice, onClose }: {
  ...
  const initial = useMemo(() => document.customUi ?? starterCustomUi(document.pages[0].id), [document.customUi, document.pages]);
  ...
  useEffect(() => { setSource(JSON.stringify(document.customUi ?? starterCustomUi(document.pages[0].id), null, 2)); }, [document.customUi, document.pages]);
  ...
  const reset = () => { const next = starterCustomUi(document.pages[0].id); setSource(JSON.stringify(next, null, 2)); setPreview(next); setError(""); };
  ...
  return <section className="custom-ui-studio">
    ...
    <div className="custom-ui-studio-body">
      <div className="custom-ui-editor-pane">
        <div className="custom-ui-editor-toolbar"><strong>ui.json</strong><span className={error ? "invalid" : "valid"}>{error ? "Fehler" : "Valide"}</span><button onClick={reset}>Starter laden</button></div>
        ...
        <p className="custom-ui-hint">Elemente: column, row, grid, card, text, value, image, spacer, button. Daten: <code>sourceId</code> + <code>path</code>.</p>
      </div>
      <div className="custom-ui-preview-pane"><div className="custom-ui-preview-label"><strong>Live Preview</strong><span>{enabled ? "Aktiv" : "Deaktiviert"}</span></div><div className="custom-ui-preview-frame"><DashboardRenderer document={{ ...document, customUi: preview }} pageIndex={0} runtime={Object.fromEntries(Object.entries(data).map(([id, value]) => [id, { value }]))} onPageChange={() => {}} /></div></div>
    </div>
  </section>;
}
