...
import type { DashboardAction, DashboardDocument, DataSource, HomeAssistantDataSource, N8nDataSource } from "../lib/dashboard";
import { Icon } from "./studio/Icons";

type Integration = {
  id: string; provider: "n8n" | "home_assistant"; name: string; base_url: string;
...
export default function IntegrationStudio({ document, onDocument, onClose, onNotice }: {
  ...
  const sources = document.dataSources.filter((source) => ((source.type === "home_assistant" || source.type === "n8n") && (!selected || source.integrationId === selected.id)) || (source.type === "action_response" && actions.some((action) => action.id === source.actionId)));
  const load = async () => {
  ...
  async function create() {
    setBusy("create");
    const credentials = form.provider === "home_assistant" ? { accessToken: form.token } : { apiKey: form.apiKey || undefined, webhookAuth: form.webhookAuth, headerName: form.headerName, headerValue: form.headerValue, username: form.username, password: form.password, jwt: form.jwt };
    const response = await fetch("/api/integrations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ provider: form.provider, name: form.name || (form.provider === "n8n" ? "n8n" : "Home Assistant"), baseUrl: form.baseUrl, credentials }) });
  ...
  return <section className="integration-studio">
    <aside className="integration-sidebar">
      ...
      <div className="integration-list">{items.map((item) => <button className={item.id === selected?.id ? "active" : ""} key={item.id} onClick={() => setSelectedId(item.id)}><strong>{item.name}</strong><small>{item.provider === "n8n" ? "n8n" : "Home Assistant"} · {item.status}</small></button>)}{!items.length && <p>Noch keine Integration.</p>}</div>
