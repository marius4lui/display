...
function widgetContent(widget: Widget, raw: unknown, history: unknown[]): ReactNode {
  ...
  if (widget.type === "gauge") {
    ...
    return <div className="widget-gauge" style={{ background: `conic-gradient(var(--widget-accent) ${percentage}%,color-mix(in srgb, var(--widget-accent) 14%, transparent) 0)` }}><span>{formatValue(raw, widget.format, widget.suffix)}</span></div>;
  }
